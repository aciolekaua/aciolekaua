<?php
 if($_SESSION['TipoCliente']=="PF"){
    if(isset($_POST['pj']) && !empty($_POST['pj'])){
        ...
        $dados=$Validate->validateGetContrato($_SESSION['ID'],$_SESSION['TipoCliente'],$_SESSION['TipoPermissao'],$Cnpj);
        ...
        if(!$dados){
            ...
            for($l=0;$l<=$lNum-1;$l++){
                ...
                if($exclusao || $conselho){
                    $cnpj=$GestaoDeConselho->getCNPJ_IdTabela($dados[$l]['ID'],"Contrato");
                    $CNPJ=$cnpj['dados']['str_empresa_Contrato'];
                ...
                echo"<td>{$dados[$l]['Historico']}</td>";
                echo"<td>{$dados[$l]['TipoDePagamento']}</td>";
                echo"<td>{$dados[$l]['Agencia']}</td>";
                echo"<td>{$dados[$l]['Conta']}</td>";
                echo"<td>{$dados[$l]['Descricao']}</td>";
                echo"<td>{$dados[$l]['Nota']}</td>";
    ...
    $dados=$Validate->validateGetContrato($_SESSION['ID'],$_SESSION['TipoCliente']);
    if(!$dados){
        //var_dump($dados);
    }else{
        ...
        $lNum=(count($dados));
        for($l=0;$l<=$lNum-1;$l++){
            $dados[$l]['Valor']=number_format($dados[$l]['v'], 2, ',', '.');
            $dados[$l]['DataDeEnvio']=date_format(date_create($dados[$l]['DataDeEnvio']),"d/m/Y H:i:s");
            $dados[$l]['Data']=date_format(date_create($dados[$l]['Data']),"d/m/Y");
            echo"<tr>";
            echo"<td><input class='form-check-input' type='checkbox' name='sel[]' value='{$dados[$l]['ID']}' /></td>";
