...
function tippy_rbt12(dados, numero){
    dados.rpa = parseFloat(dados.rpa);
    dados.rba = parseFloat(dados.rba);
    dados.rbt12 = parseFloat(dados.rbt12);
    //console.log(dados);
    tippy('#rbt12_tippy_' + numero, {
        content: `
            <h4 style="color:#d1cccc">RBA:  
                <span style="color: #fff;">${dados.rba.toLocaleString('pt-BR', {
                    style: 'currency',
                    currency: 'BRL',
                    minimumFractionDigits: 2
                })}</span>
            </h4>
            <h4 style="color:#d1cccc;">RPA: 
                <span style="color: #fff;">${dados.rpa.toLocaleString('pt-BR', {
                    style: 'currency',
                    currency: 'BRL',
                    minimumFractionDigits: 2
                })}</span>
            </h4 style="color:#d1cccc">
            <h4 style="color:#d1cccc">RBT12: 
                <span style="color: #2fb490;">${dados.rbt12.toLocaleString('pt-BR', {
                    style: 'currency',
                    currency: 'BRL',
                    minimumFractionDigits: 2
                })}</span>
...
function escrever_tabela(data){
    ...
    for(let i=0;i<=(data.length - 1);i++){
        ...
        if(data[i].rbt12 > 2160000){
            c_rbt12.querySelector('p').setAttribute('class','rbt12 limite2');
        }
        c_rbt12.setAttribute('class','text-matrix');
        c_rbt12.setAttribute('id','rbt12_tippy_' + i);
        c_rbt12.addEventListener('mouseover',()=>{
            tippy_rbt12(data[i], i);
        });
        row.append(c_rbt12);
    ...
    start_tippys();
    /*{
        'rpa':data.rpa.rbt12.toLocaleString('pt-BR', {
            style: 'currency',
            currency: 'BRL',
            minimumFractionDigits: 2
        }),
        'rba':data.rba.rbt12.toLocaleString('pt-BR', {
            style: 'currency',
            currency: 'BRL',
            minimumFractionDigits: 2
        })
...
window.addEventListener("load",()=>{
    ...
    document.querySelector('input[name=rbt12]').addEventListener('blur',()=>{
        calcularRBT12();
    });

    document.querySelector('input[name=folha]').addEventListener('blur',()=>{
        calcularRBT12();
    });

    console.log(document.querySelector('input[name=rbt12]').id);
    console.log(document.querySelector('input[name=folha]').id);
})