...
function calcularRBT12(dados){
    ...
    for(let anexo=1;anexo<=5;anexo++){
        ...
        if(!input_faturamento_sem_retencao == 0 || !input_faturamento_com_retencao == 0){
            ...
            $.ajax({
                url:getRoot('calcular-simples/calcularSimples'),
                type:"POST",
                dataType:'json',
                data: formData,
                timeout:12000,
                ...
                success :function(data,status,xhr){
                    ...
                    if(data.anexoOriginal==5){
                       // h2_folha_card.innerText = "Fator R: " + data.fatorR + '%';
                       console.log(h2_folha_card);
                    }
                   console.log(data.fatorR.replace('.',','));
                    ...
                    document.querySelector('h3[id=div_valor_totalizado_anexo_'+anexo+']').innerText = "Anexo "+anexo+": " + valor_total_DAS.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
            ...
            document.querySelector('h3[id=div_valor_totalizado_anexo_'+anexo+']').innerText = "Anexo "+anexo+": R$ 0,00";
            /*document.querySelector('h3[id=div_aliquota_retencao_anexo_'+anexo+']').innerText = "Anexo "+anexo+": 0%";
...
function getFatulras(dados){
    ...
    $.ajax({
        ...
        success: function(data,status,xhr){
            ...
            valor_faturamento_sem_rentencao_total = 0;
            valor_faturamento_com_rentencao_total = 0;
            ...
            for(let i=1;i<=5;i++){
                ...
                document.querySelector('h3[id=div_valor_faturamento_sem_rentencao_anexo_'+i+']').innerText = "Anexo "+i+":"+faturamento_sem_retencao[i].toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
                document.querySelector('h3[id=div_valor_faturamento_com_rentencao_anexo_'+i+']').innerText = "Anexo "+i+":"+faturamento_com_retencao[i].toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
            }

            document.querySelector('h4[id=div_valor_faturamento_sem_rentencao_total]').innerText = "Total do Faturamento Sem Retenção: "+valor_faturamento_sem_rentencao_total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
            document.querySelector('h4[id=div_valor_faturamento_com_rentencao_total]').innerText = "Total do Faturamento Com Retenção: "+valor_faturamento_com_rentencao_total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
...
function getRBT12(dados){
    ...
    $.ajax({
        ...
        success :function(data,status,xhr){
            ...
            if(data.dados!=false){
                let rba = parseFloat(data.dados.rba);
                
                h2_rba_card.innerText ="Total do RBA: " +  rba.toLocaleString('pt-BR', {
                    style: 'currency',
                    currency: 'BRL',
                    minimumFractionDigits: 2
                });
                ...
                if(rba > 0){
                    ...
                    let part2 = (dados.rba * 30) / 100;
                    let part3 = (dados.rba * 10) / 100;*/
                    limite.data.datasets[0].needleValue = rba;
                    limite.update();
                    //console.log(limite);
                }
