function excluirArquivo(dados){
    ...
    formData.append('idGrupo',dados.grupo);

    console.log(dados);

    $.ajax({
        url:getRoot(getURL()+'/excluirArquivo'),
        type:"POST",
        dataType:'json',
        data: formData,
        timeout:12000,
...
function escreverTabelaPJ(data, associados=null){
    ...
    /*for(let i=0;i<=data.conselheiros.length-1;i++){
        if(data.conselheiros[i].NomePF!=undefined){
            let arrayNome =  data.conselheiros[i].NomePF.split(' ');
            data.conselheiros[i].NomePF = `${arrayNome[0]} ${arrayNome[1]}`;
            theadTabela.innerHTML+=`<th id='th${data.conselheiros[i].CPF}'>${data.conselheiros[i].NomePF}</th>`;
        }
    }
    theadTabela.innerHTML+="<th></th>";*/
