...
function escreverTabelaPJ(data,associados=null){
    ...
    for(let i=0;i<=(data.tabela.length-1);i++){
        ...
        if(data.tabela[i].arquivos.length>0){
            //arquivos = `<a target='_blank' href='${data.tabela[i].Arquivo}'>${data.tabela[i].Arquivo}</a>`;
            arquivos = "<button type='button' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#modalArquivos'>Arquivos</button>";
        }else{
