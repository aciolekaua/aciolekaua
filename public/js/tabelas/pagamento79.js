...
function escreverListaArquivos(data){
    ...
    /*var listaArquivos = document.querySelector("div[id=lista-comprovante]");
    listaArquivos.innerHTML='';
    for(let i=0;i<=data.length-1;i++){
        //listaArquivos.innerHTML+="";
        var item = document.createElement("a");
        item.setAttribute('class','list-group-item list-group-item-action');
        item.setAttribute('href',data[i].url);
        item.setAttribute('download',data[i].nome);
        if(data[i].extensao=="pdf"){
            item.innerHTML= "<i class='bi bi-file-pdf h4 text-danger me-2'></i>"+data[i].nome;
        }else if(
            data[i].extensao=="png"
            || data[i].extensao=="jpeg"
            || data[i].extensao=="jpg"
        ){
            item.innerHTML= "<i class='bi bi-file-image h4 text-primary me-2'></i>"+data[i].nome;
        }else{
            item.innerHTML= "<i class='bi bi-question-square h4 text-secondary me-2'></i>"+data[i].nome;
        }
        //item.innerText=data[i].nome;
        listaArquivos.append(item);
        /*<a class="list-group-item list-group-item-action" href="https://ivici.com.br/public/img/lancamentos/pagamentos/668c2de6b230d.pdf" download >
        <i class="bi bi-filetype-pdf h4 text-danger"></i> 
        An item</a>
    }*/
