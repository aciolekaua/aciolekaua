function excluirArquivo(dados){
    ...
    $.ajax({
        ...
        success: function(data,status,xhr){
            console.log(data);
            tabelaPJ();
            console.log(document.querySelector(`button[id-lancamento=${dados.id}]`));
        },
...
function adicionarImagem(dados){
    ...
    $.ajax({
        ...
        success: function(data,status,xhr){
            console.log(data);
            tabelaPJ();
            document.querySelector(`button[id-lancamento=${dados.id}]`).click();
        },
        error: function(jqXhr, textStatus, errorMessage){
            console.log(jqXhr+" "+textStatus+" "+errorMessage);
    ...
    for(let i=0;i<=(data.tabela.length-1);i++){
        ...
        if(data.tabela[i].arquivos.length>0){
            ...
            arquivos = `<button 
                ...
                id-lancamento = '${data.tabela[i].ID}'
