...
window.addEventListener('load',()=>{
    ...
    for(let i=0;i<=6;i++){
        document.querySelectorAll('select[name=ano]')[0].innerHTML+=`<option value="${(ano-i)}">${(ano-i)}</option>`;
        document.querySelectorAll('select[name=ano]')[1].innerHTML+=`<option value="${(ano-i)}">${(ano-i)}</option>`;
    }
    buscarExtrato();
});
