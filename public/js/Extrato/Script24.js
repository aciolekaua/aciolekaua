             ...
             if(data.erros.length>0){
                ...
                if(data.dados.length<=0){
                    ...
                    for(let i=0;i<=data.dados.length-1;i++){
                        ...
                        var json = `{
                            'competencia':'${data.dados[i].dataCompetencia}',
                            'id':'${data.dados[i].id}',
                            'cnpj':'${data.dados[i].cnpj}'
                        }`;
                        ...
                        buttons+=`
                            <div class='col'>
                                ...
                                <i class="btn bi bi-pencil-square text-primary" data-bs-toggle="modal" data-bs-target="#modalEditarExtrato" onclick="setModalUpdateExtrato(${json});"></i>
                                
                            </div>
                        `;
                        buttons+=`<div class='col'><i data-bs-toggle="modal" data-bs-target="#modalConfirmaExclusaoExtrato" onclick="setModalExclusaoExtrato('${data.dados[i].id}');" class="btn bi bi-trash-fill text-danger"></i></div>`;
                        buttons+=`</div>`;
...
function updateExtrato(){
    let formData = new FormData($('#formUpdateExtrato')[0]);
    //formData.append('id',id);
    $.ajax({
        url:getRoot(getURL()+'/updateExtrato'),
        beforeSend:function(){
            $('.overlayk-loading').fadeIn();
        },
        type:"POST",
...
function setModalUpdateExtrato(json){
    document.querySelector('input[name=IdExtrato]').value=json.id;
    document.querySelector('input[name=cnpjEditar]').value=json.cnpj;
    console.log(json);
    document.querySelector('button[id=btnEditarExtrato]').addEventListener('click',()=>{
        updateExtrato();
    });

}

function setModalExclusaoExtrato(id=null){
    if(id!=null){
        document.querySelector("button[id=btnConfirmaExclusaoExtrato]")
