    ...
    $.ajax({
        ...
        success: function(data, status, xhr) {
            ...
            if (data.TipoCliente == "PF") {
                ...
                let select = document.querySelector('select[name=PJ]');
                select.innerHTML = "";
                if(data.PJs!=false){
                    for(let i=0; i<=(data.PJs.length -1); i++){
                        select.innerHTML+=`<option value="${data.PJs[i].CNPJ}">${data.PJs[i].Nome}</option>`;
                    }
                }

                select.addEventListener('change',()=>{buscarDadosPJ();});
                buscarDadosPJ();
...
function atualizarIndicatorsDiferenca(dadosAnuais, seletorAnalyze) {
    ...
    if (ultimoMes && mesAnteriorItem) {
        ...
        indicator.className = diferenca >= 0 ? 'verde' : 'vermelho';
...
const tres = document.getElementById('myCharttres');  

let mycharttres = new Chart(tres, {
    type: 'bar',
    data: {
...
const quatro = document.getElementById('myChartquatro');

let myChartquatro = new Chart(quatro, {
  type: 'doughnut',
  data: {
